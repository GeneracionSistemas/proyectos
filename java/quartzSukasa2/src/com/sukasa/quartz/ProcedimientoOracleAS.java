...
public class ProcedimientoOracleAS {
	...
	public static StringBuilder procesarORACLE(StringBuilder sentencia) {
		...
		try {
			...
			String queryProgramasPortal = "SELECT CONCAT(CONCAT(IMARCA, '-'),ICOD) as SKU, DESCRIPCION FROM programa_portal IMARCA = 1729 AND ICOD IN (922,923,924,926,927,928)";
...
//				System.out.println("TOTAL KARDEX:" + kardex.size());
			...
			// "select * from maestro_tarjeton where MTMARCA=3289 and MTITEM=312";
			 ...
			 String queryMaestroTarjeton =
			 "select * from maestro_tarjeton where MTMARCA=3664 and MTITEM=8";
...
//			String queryProgramasPortal = "SELECT CONCAT(CONCAT(IMARCA, '-'),ICOD) as SKU, DESCRIPCION FROM programa_portal where SKU IN ('1729-922','1729-923','1729-924','1729-926','1729-927','1729-928')";
//			ResultSet rsQueryProgramasPortal = instruccion
//					.executeQuery(queryProgramasPortal);
//			List<Programa> lstProductosPrograma = new ArrayList<Programa>();
//			while (rsQueryProgramasPortal.next()) {
						...
						for (DescuentoMesesPromocion descuentoMesesPromocion : listDescuentoMesesPromocion) {
							if (paramTarjeton.getPt_nmeses() == descuentoMesesPromocion
								...
								if (descuentoMesesPromocion.getDescuento() == 0) {
									double valorCuota = round(maestro_tarjeton
											.getMtPrecio()
											/ descuentoMesesPromocion
													.getMeses(),2);
									double valorMensual = round(valorCuota
											* descuentoMesesPromocion
													.getMeses(),2);
									...
									double cuota = maestro_tarjeton.getMtPrecio() * factor;

									double valorMensual = maestro_tarjeton.getMtPrecio() - cuota;
									valorMensual = round(valorMensual,2);
									double valorCuota = round(valorMensual/descuentoMesesPromocion.getMeses(),2);
									valorMensual = round(valorCuota*descuentoMesesPromocion.getMeses(),2 );
...
//							"root", "");
						...
						if(existe_tendencia_programa){
							...
							instruccion
									.executeUpdate("delete from catalog_category_product_index where category_id="
											+ id_categoria_magento_programa
											+ " and product_id=" + id_producto + "");
							quitarSubTendencia(id_producto, id_categoria_magento_programa, id_categoria_tendencia);
						}
						...
						if (!existe_tendencia_programa_en_padre) {
							...
							instruccion
									.executeUpdate("insert into catalog_category_product_index(category_id,product_id,position,is_parent,store_id,visibility)values("
											...
											+ ","
											+ id_padre + ",1,1,1,4)");
							asignarSubTendencia(id_producto, id_categoria_magento_programa, id_categoria_tendencia);
	...
	public static int getPadreId(int producto_id){
		...
		try {
			...
			if(padre_id > 0){
				System.out
				.println("Se encontró un producto configurable en el producto: " + producto_id );
			}
		} catch (SQLException e) {
			System.out.println("Error de MySQL: " + e.getMessage());
		} catch (Exception e) {
	...
	public static void quitarSubTendencia(int producto_id, int tendencia_id, int tendencia_principal_id){
		...
		try {
			...
			ResultSet rsQuerySubCategorias = instruccion
					.executeQuery("SELECT entity_id FROM catalog_category_entity WHERE  (path LIKE '1/2/"
							+ tendencia_principal_id + "/" + tendencia_id + "/%') AND (level <= '4')");
			while (rsQuerySubCategorias.next()) {
				tendencia_cats = tendencia_cats + rsQuerySubCategorias.getInt("entity_id") + ",";
			}
			
			if (tendencia_cats.length() > 0 && tendencia_cats.charAt(tendencia_cats.length()-1)==',') {
				tendencia_cats = tendencia_cats.substring(0, tendencia_cats.length()-1);
			}
			...
			while (rsQueryProdCategorias.next()) {
				productCategories = productCategories + rsQueryProdCategorias.getInt("category_id") + ",";
			}
			if (productCategories.length() > 0 && productCategories.charAt(productCategories.length()-1)==',') {
				productCategories = productCategories.substring(0, productCategories.length()-1);
			}
			
			String[] tendencia_cats_array = tendencia_cats.split(",");
			List<String> tendencia_cats_List = Arrays.asList(tendencia_cats_array);
			
			String[] productCategories_array = productCategories.split(",");
			
			List<String> aList = Arrays.asList(productCategories_array);
			List<String> bList = Arrays.asList(topCategories_array);
			aList.retainAll(bList);
			//System.out.println(" a intersection b "+aList);
			//String[] categoriesFound = aList.toArray(new String[0]);
			...
			if(aList.size() > 0)
			{
				for(String Id: aList){
					...
					while (rsQueryCategoriaNombre.next()) {
						categoriaNombre = rsQueryCategoriaNombre.getString("value");
					}
					...
					if(categoriaNombre != ""){
						...
						for(String SubId: tendencia_cats_List){
							...
							while (rsQuerySubCategoriaNombre.next()) {
								categoriaNombre = rsQuerySubCategoriaNombre.getString("value");
							}
							
							if(SubcategoriaNombre == categoriaNombre)
							{
								
								instruccion
								.executeUpdate("delete from catalog_category_product where category_id="
										+ SubId
										+ " and product_id=" + producto_id + "");
								instruccion
										.executeUpdate("delete from catalog_category_product_index where category_id="
												+ SubId
												+ " and product_id=" + producto_id + "");
	...
	public static void asignarSubTendencia(int producto_id, int tendencia_id, int tendencia_principal_id){
		String topCategories = "3,139,140,141,142,143,144,145,147,679";
		String[] topCategories_array = topCategories.split(",");
