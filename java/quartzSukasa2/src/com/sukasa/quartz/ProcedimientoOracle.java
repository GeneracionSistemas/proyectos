...
public class ProcedimientoOracle {
	...
	public static StringBuilder procesarORACLE(StringBuilder sentencia) {
		...
		try {
			...
			// "select * from maestro_tarjeton where MTMARCA=3152 and MTITEM=80";
			...
			// categorías de programas del portal

			// String queryProgramasPortal =
			// "SELECT CONCAT(CONCAT(IMARCA, '-'),ICOD) as SKU, DESCRIPCION FROM programa_portal where IMARCA=3868 and ICOD in (55,56,57,58,59,60,61,62)";
	...
	public static void actualizarTarjetonProductos(
		...
		try {
			...
			String textoAfiliadoContadoPromocion = "AFILIADO CONTADO PROMOCIÓN: $ ";

			for (MaestroTarjeton maestro_tarjeton : listMaestroTarjeton) {
				Date fechaActual = new Date();
				DateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd");
				// String dfecha1 = maestro_tarjeton.getMtFechaDesde();
				Date dFecha1 = maestro_tarjeton.getMtFechaDesde1();
				// String dfecha2 = maestro_tarjeton.getMtFechaHasta();
				Date dFecha2 = maestro_tarjeton.getMtFechaHasta1();
				...
				if (grupo != 0) {
					...
					if (listDescuentoMesesPromocion.size() > 0) {
							...
							for (DescuentoMesesPromocion descuentoMesesPromocion : listDescuentoMesesPromocion) {
								if (paramTarjeton.getPt_nmeses() == descuentoMesesPromocion
									...
									if (descuentoMesesPromocion.getDescuento() == 0) {
										...
										double valorMensual = redondear(valorCuota
												* descuentoMesesPromocion
														.getMeses());
										...
										double valorCuota = valorMensual
												/ descuentoMesesPromocion
														.getMeses();
										valorCuota = redondear(valorCuota);
										valorMensual = redondear(valorCuota
												* descuentoMesesPromocion
														.getMeses());
						...
						tarjeton = "<div class=" + comillas + "stdCuotas"
								+ comillas + "><div class=" + comillas
								+ "formUno" + comillas + "><span class="
								+ comillas + "code" + comillas + "> CODIGO: "
								+ maestro_tarjeton.getMtCodigo() + "-"
								+ maestro_tarjeton.getMtItem()
								+ "</span><span class=" + comillas + "code"
								+ comillas + "> MODELO: "
								+ maestro_tarjeton.getMtModelo()
						...
						if (!maestro_tarjeton.getMtFechaDesde().toString()
							...
							fechasPromocion = "<p class=" + comillas + "code"
									+ comillas + ">Del: "
									+ maestro_tarjeton.getMtFechaDesde()
									+ " al "
						...
						if (fechaActual.after(dFecha1)
							...
							precioOferta = "<p class=" + comillas + "code"
									...
									+ "</p>" + "<p id=" + comillas + "PVP"
									+ comillas + ">"
									+ textoAfiliadoContadoPromocion
									+ maestro_tarjeton.getMtPrecioOferta()
									+ "</p>";
						} else {
							precioOferta = "<p class=" + comillas + "code"
									+ comillas + ">"
						...
						tarjeton = tarjeton + "</div>" + precioOferta
								+ "<p class=" + comillas + "code" + comillas
								+ ">" + descripcionPromo + "</p>"
								+ fechasPromocion + "<p class=" + comillas
								+ "code" + comillas + "><br/><p class="
								+ comillas + "code" + comillas + ">" + cuotas
								+ "</p><table class=" + comillas + "Tabcuotas"
								+ comillas + "><tr>";
	...
	public static void insertDestacadoWeb(List<DestacadosWeb> listScriptInserts) {
		...
		try {
			...
			for (DestacadosWeb destacadosWeb : listScriptInserts) {
				boolean empty = true;
				String queryDestacadosIngresados = "select DMARCA, DITEM from _destacados_web where DMARCA='"
						+ destacadosWeb.getDmarca()
						+ "' and DITEM='"
						+ destacadosWeb.getDitem() + "' and DACTUALIZADO='N'";
				ResultSet rsDestacadosIngresados = instruccion
						.executeQuery(queryDestacadosIngresados);
				while (rsDestacadosIngresados.next()) {
					empty = false;
					// update
					instruccion1
							.executeUpdate("update _destacados_web set DCATEGORIA='"
				...
				if (empty == true) {
					// insert
					instruccion1
							.executeUpdate("insert into _destacados_web(DMARCA, DITEM, DCATEGORIA, DFECHAI, DFECHAF, DFECHA_INGRESO, DDESTACADO,DACTUALIZADO) values("
									+ destacadosWeb.getDmarca()
									+ ","
									+ destacadosWeb.getDitem()
									+ ",'"
	...
	public static void habilitar_programas_portal(
		...
		try {
			...
			for (Programa programa : productosPrograma) {
				...
				boolean existe_tendencia_en_padre = false;
				boolean existe_tendencia_programa = false;
				boolean existe_tendencia_programa_en_padre = false;
				...
				// Obtener el configurable donde reside el producto
				int id_padre = getPadreId(id_producto);
				...
				} else if (programa.getDescripcion().equals("LIVING THE AUTH")) {
					id_categoria_magento_programa = 4835;
				} else if (programa.getDescripcion().equals("BAZAR TROPICAL")) {
					id_categoria_magento_programa = 4836;
				}
				...
				if (id_producto != 0 && id_categoria_magento_programa != 0) {
					...
					if (id_padre > 0) {
						...
						ResultSet rsQueryTendenciasPadre1 = instruccion
								.executeQuery("SELECT * FROM catalog_category_product WHERE category_id="
										+ id_categoria_tendencia
										+ " AND product_id=" + id_padre + "");
						while (rsQueryTendenciasPadre1.next()) {
							existe_tendencia_en_padre = true;
							break;
						}

						if (!existe_tendencia_en_padre) {
					...
					if (!existe_tendencia && id_padre == 0) {
					...
					if (id_padre > 0) {
						if (existe_tendencia_programa) {
							...
							instruccion
									.executeUpdate("delete from catalog_category_product_index where category_id="
											...
											+ id_producto
											+ "");
							quitarSubTendencia(id_producto,
									id_categoria_magento_programa,
									id_categoria_tendencia);
						}
						...
						ResultSet rsQueryTendenciasPadre1 = instruccion
								.executeQuery("SELECT * FROM catalog_category_product WHERE category_id="
										...
										+ " AND product_id=" + id_padre + "");
						while (rsQueryTendenciasPadre1.next()) {
							existe_tendencia_programa_en_padre = true;
							break;
						}

						if (!existe_tendencia_programa_en_padre) {
							...
							instruccion
									.executeUpdate("insert into catalog_category_product_index(category_id,product_id,position,is_parent,store_id,visibility)values("
											+ id_categoria_magento_programa
											+ "," + id_padre + ",1,1,1,4)");
							asignarSubTendencia(id_producto,
									id_categoria_magento_programa,
									id_categoria_tendencia);
	...
	public static double redondear(double value) {
		...
		double valorDouble = Double.parseDouble(big.toString());
		return valorDouble;
	}

	public static int getPadreId(int producto_id) {
		int padre_id = 0;
		Connection conexion = null;
		try {
			Class.forName("com.mysql.jdbc.Driver");
			...
			ResultSet rsQueryCodigos = instruccion
					.executeQuery("SELECT parent_id FROM catalog_product_super_link WHERE product_id IN("
							+ producto_id + ")");
			while (rsQueryCodigos.next()) {
				padre_id = rsQueryCodigos.getInt("parent_id");
			}
		} catch (SQLException e) {
			System.out.println("Error de MySQL: " + e.getMessage());
		} catch (Exception e) {
	...
	public static void quitarSubTendencia(int producto_id, int tendencia_id,
			int tendencia_principal_id) {
		String topCategories = "3,139,140,141,142,143,144,145,147,679";
		String[] topCategories_array = topCategories.split(",");

		String tendencia_cats = "";
		...
		try {
			...
			String query = "SELECT entity_id FROM catalog_category_entity WHERE  (path LIKE '1/2/"
					+ tendencia_principal_id
					+ "/"
					+ tendencia_id
					+ "/%') AND (level <= '4')";

			if (tendencia_id == 4835 || tendencia_id == 4836) {

				query = "SELECT entity_id FROM catalog_category_entity WHERE  (path LIKE '1/2/"
						+ tendencia_principal_id
						+ "/4906/"
						+ tendencia_id
						+ "/%') AND (level <= '5')";
			...
			ResultSet rsQuerySubCategorias = instruccion.executeQuery(query);
			while (rsQuerySubCategorias.next()) {
				tendencia_cats = tendencia_cats
						+ rsQuerySubCategorias.getInt("entity_id") + ",";
			}

			if (tendencia_cats.length() > 0
					&& tendencia_cats.charAt(tendencia_cats.length() - 1) == ',') {
				tendencia_cats = tendencia_cats.substring(0,
						tendencia_cats.length() - 1);
			}

			ResultSet rsQueryProdCategorias = instruccion
					.executeQuery("SELECT catalog_category_product.category_id FROM catalog_category_product WHERE (product_id = "
							+ producto_id + ")");
			while (rsQueryProdCategorias.next()) {
				productCategories = productCategories
						+ rsQueryProdCategorias.getInt("category_id") + ",";
			}
			if (productCategories.length() > 0
					&& productCategories.charAt(productCategories.length() - 1) == ',') {
				productCategories = productCategories.substring(0,
						productCategories.length() - 1);
			}

			String[] tendencia_cats_array = tendencia_cats.split(",");
			List<String> tendencia_cats_List = Arrays
					.asList(tendencia_cats_array);

			String[] productCategories_array = productCategories.split(",");

			List<String> aList = new ArrayList<String>(
					Arrays.asList(productCategories_array));
			List<String> bList = Arrays.asList(topCategories_array);
			aList.retainAll(bList);

			if (aList.size() > 0) {
				for (String Id : aList) {
					String categoriaNombre = "";
					ResultSet rsQueryCategoriaNombre = instruccion
							.executeQuery("SELECT value FROM catalog_category_entity_varchar WHERE attribute_id = 33 AND entity_id = "
									+ Id);
					while (rsQueryCategoriaNombre.next()) {
						categoriaNombre = rsQueryCategoriaNombre
								.getString("value");
					}

					if (categoriaNombre != "") {

						for (String SubId : tendencia_cats_List) {
							String SubcategoriaNombre = "";
							ResultSet rsQuerySubCategoriaNombre = instruccion
									.executeQuery("SELECT value FROM catalog_category_entity_varchar WHERE attribute_id = 33 AND entity_id = "
											+ SubId);
							while (rsQuerySubCategoriaNombre.next()) {
								SubcategoriaNombre = rsQuerySubCategoriaNombre
										.getString("value");
							}

							if (SubcategoriaNombre.equals(categoriaNombre)) {
	...
	public static void asignarSubTendencia(int producto_id, int tendencia_id,
			int tendencia_principal_id) {
		String topCategories = "3,139,140,141,142,143,144,145,147,679";
		String[] topCategories_array = topCategories.split(",");
