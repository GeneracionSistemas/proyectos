...
public class Copy_3_of_ProcedimientoOracle {
	...
	public static StringBuilder procesarORACLE(StringBuilder sentencia) {
		...
		try {
			...
			/*
			 * Scripts para insertar la informacion de la tabla temporal
			 * _maestro_tarjeton
			 */
			...
			// ya no se usa esta tabla para los calculos
			...
			// "SELECT CONCAT(CONCAT(IMARCA, '-'),ICOD) as sku, DESCRIPCION FROM programa_portal where DESCRIPCION='CUPCAKES' AND IMARCA in(8,8) and ICOD in(180,194)";
...
//							"root", "yagehosting");
			...
			//String queryMaestroTarjeton = "select * from _maestro_tarjeton where precio>0 and marca=3739 and item=4";
			//String queryMaestroTarjeton = "select * from _maestro_tarjeton where precio>0 and marca=1650 and item=316";
			ResultSet rsQueryMaestroTarjeton = instruccion
					.executeQuery(queryMaestroTarjeton);
			...
			while (rsQueryMaestroTarjeton.next()) {
				...
				DateFormat dateFormat = new SimpleDateFormat("yyyy-mm-dd");
				Date dfecha1 = rsQueryMaestroTarjeton.getDate("FECHA_DESDE");
				dateFormat.format(dfecha1);
				Date dfecha2 = rsQueryMaestroTarjeton.getDate("FECHA_HASTA");
				dateFormat.format(dfecha2);
				...
				if (listDescuentoMesesPromocion.size() > 0) {
							...
							if (paramTarjeton.getPt_nmeses() == descuentoMesesPromocion
								...
								if (descuentoMesesPromocion.getDescuento() == 0) {
									...
									if (valorCuota >= paramTarjeton
										...
										numeroMesesTarjeton.add(descuentoMesesPromocion.getMeses());
										...
										if (grupo == 1) {
											...
											cuotasProductoTarjeton = 
													...
													+ df.format(valorMensual)
													+ "</p></td>";
											cuotasTarjetonPublicar.add(cuotasProductoTarjeton);
										} else if (grupo == 3) {
											cuotasProductoTarjeton = 
													"<td><p>"
													+ descuentoMesesPromocion
					...
					if(cuotaResaltarProductoTarjeton.length()>0){
						...
						for (String string : cuotasTarjetonPublicar) {
							if(k<numeroMesesTarjeton.size()-1){
								cuotasProductoTarjeton = cuotasProductoTarjeton+string.toString();
								k++;
							}
						}
					}
					...
					tarjeton = "<div class=" + comillas + "stdCuotas"
							...
							+ comillas + "> CODIGO: "
							+ rsQueryMaestroTarjeton.getInt("MARCA") + "-"
							+ rsQueryMaestroTarjeton.getInt("ITEM")
							...
							+ ">PVP: USD$ "
							+ rsQueryMaestroTarjeton.getDouble("PRECIO")
							+ "</p>";
					...
					if (!rsQueryMaestroTarjeton.getDate("FECHA_DESDE")
							.toString().equals("1999-06-06")) {
					...
					if (fechaActual.after(dfecha1)
						...
						precioOferta = "<p class="+ comillas+ "code"+ comillas+ ">"
								+ rsQueryMaestroTarjeton
										.getString("NOMBRE_PRECIO_OFERTA") + "</p>";
					}
					
					tarjeton = tarjeton
							+ "</div>"+precioOferta+"<p class=" + comillas + "code" + comillas
			...
			System.out
			.println("Inserto y actualizo tarjeton de productos");
		} catch (SQLException e) {
			System.out.println("Error de MySQL: " + e.getMessage());
		} catch (Exception e) {
			System.out.println("Error inesperado: " + e.getMessage());
...
//			 "root", "yagehosting");
	...
	public static void urlsDuplicadas() {
		...
		try {
			...
			String querySelect = "SELECT value FROM catalog_product_entity_varchar WHERE attribute_id =86 GROUP BY value HAVING COUNT( * ) >1 ";
