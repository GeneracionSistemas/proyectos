...
public class ProcedimientoOracle_Actual {
	...
	public static StringBuilder procesarORACLE(StringBuilder sentencia) {
		...
		try {
			 ...
			 * listItemsInsertarOracle.size() > 0) { String queryTruncate =
			 * "truncate table items"; instruccion.executeQuery(queryTruncate);
			 * for (String string : listItemsInsertarOracle) {
			 * instruccion.executeUpdate(string.toString()); } System.out
			 * .println
			 * ("Actualizo e inserto productos en tabla items de sukasa"); }
			 */
			...
			ResultSet rsQueryPlazoMonto = instruccion
					.executeQuery(queryPlazoMonto);
			...
			while (rsQueryPlazoMonto.next()) {
				...
				plazoMonto.setMontoMinimo(rsQueryPlazoMonto
						.getDouble("MONTO_MINIMO"));
			...
			// Saca los items filtrados en la tabla maestro_tarjeton para
			// ocultar en el portal web los que no se encuentran en esta lista
			String queryMaestroTarjetonProductosBloqueados = "select mtmarca, mtitem from maestro_tarjeton";

			ResultSet rsQueryMaestroTarjetonProductosBloqueados = instruccion
					.executeQuery(queryMaestroTarjetonProductosBloqueados);

			List<MaestroTarjeton> listProductosBloqueados = new ArrayList<MaestroTarjeton>();

			while (rsQueryMaestroTarjetonProductosBloqueados.next()) {
				MaestroTarjeton productoBloqueado = new MaestroTarjeton();
				productoBloqueado
						.setMtCodigo(rsQueryMaestroTarjetonProductosBloqueados
								.getInt("MTMARCA"));
				productoBloqueado
						.setMtItem(rsQueryMaestroTarjetonProductosBloqueados
								.getInt("MTITEM"));
				listProductosBloqueados.add(productoBloqueado);
			}

			if (listProductosBloqueados.size() > 0) {
				deshabilitarBloqueados(listProductosBloqueados);
			}
	...
	public static void ocultarProductosCantidadCero() {
		...
		try {
			...
			ResultSet rsQuery = instruccion
					.executeQuery("SELECT csi.product_id FROM catalog_product_entity cpe, cataloginventory_stock_item csi WHERE cpe.entity_id = csi.product_id AND cpe.sku NOT LIKE 'receta%' AND cpe.sku NOT LIKE 'secre%'AND csi.qty =0");
			while (rsQuery.next()) {
	...
	public static void actualizarTarjetonProductos(
			...
			List<PlazoMonto> listPlazoMonto) {
		...
		try {
			...
			//String queryMaestroTarjeton ="SELECT * FROM _maestro_tarjeton WHERE codigo=1280 or codigo=476";
			// String queryMaestroTarjeton =
			// "SELECT * FROM _maestro_tarjeton WHERE codigo=1315";
			ResultSet rsQueryMaestroTarjeton = instruccion
					.executeQuery(queryMaestroTarjeton);
			...
			while (rsQueryMaestroTarjeton.next()) {
				...
				for (PlazoMonto plazoMonto : listPlazoMonto) {
					if (grupo == 1) {
						if (plazoMonto.getMes() == 3
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
						...
						if (plazoMonto.getMes() == 3
							...
							calcularCuota3Meses = true;
							numeroCuotas.add(3);
						} else if (plazoMonto.getMes() == 6
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
							calcularCuota6Meses = true;
							numeroCuotas.add(6);
						} else if (plazoMonto.getMes() == 9
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
							calcularCuota9Meses = true;
							numeroCuotas.add(9);
						} else if (plazoMonto.getMes() == 12
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
						...
						if (plazoMonto.getMes() == 9
							...
							calcularCuota12Meses = true;
							numeroCuotas.add(12);
						} else if (plazoMonto.getMes() == 15
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
							calcularCuota15Meses = true;
							numeroCuotas.add(15);
						} else if (plazoMonto.getMes() == 18
								&& rsQueryMaestroTarjeton.getDouble("PRECIO") >= plazoMonto
										.getMontoMinimo()) {
				...
				if (codPromocionMT != 0) {
					for (Promocion promocionlista : listPromocion) {
						if (promocionlista.getPcod() == codPromocionMT) {
							for (int i = 0; i < numeroCuotas.size(); i++) {
								if (promocionlista.getPmes() == numeroCuotas
										.get(i)) {
					...
					for (Fpmes fpmes : listFpmes) {
						for (Integer integer : numeroCuotas) {
					...
					for (DescuentoMesesPromocion descuentoMesesPromocion : listDescuentoMesesPromocion) {
						...
						if (descuentoMesesPromocion.getDescuento() == 0) {
							...
							if (descuentoMesesPromocion.getMeses() == 3
									|| descuentoMesesPromocion.getMeses() == 6
									|| descuentoMesesPromocion.getMeses() == 9
									|| descuentoMesesPromocion.getMeses() == 12
									|| descuentoMesesPromocion.getMeses() == 15
							...
							if (grupo == 1) {
								...
								if (descuentoMesesPromocion.getMeses() != 12) {
									...
									if (calcularCuota3Meses == true
											|| calcularCuota6Meses == true
											|| calcularCuota9Meses == true) {
										if(i<listDescuentoMesesPromocion.size()){
											cuotasProductoTarjeton = cuotasProductoTarjeton
													+ "<td><p>"
													+ descuentoMesesPromocion
													.getMeses()
				...
				if (calcularCuota3Meses == true || calcularCuota6Meses == true
						|| calcularCuota9Meses == true
						|| calcularCuota12Meses == true
						|| calcularCuota15Meses == true
						|| calcularCuota18Meses == true) {
	...
	public static void deshabilitarBloqueados(
		...
		try {
			...
			ResultSet rsQuery = instruccion
					.executeQuery("SELECT entity_id FROM catalog_product_entity WHERE sku NOT IN ("
							+ codigosBloqueados + ") and sku not like 'receta%' and sku not like 'secre%'");
			while (rsQuery.next()) {
				Statement instruccion1 = conexion.createStatement();
				instruccion1
						.executeUpdate("update catalog_product_entity_int set value=2 where entity_id="
			...
			System.out.println("Se oculto productos bloqueados en el portal");
		} catch (SQLException e) {
			System.out.println("Error de MySQL: " + e.getMessage());
		} catch (Exception e) {
			System.out.println("Error inesperado: " + e.getMessage());
